...
model {
    ...
    //mu ~ normal(0, .5);
    diag_stds ~ normal(0, 1.0);
    noise_std ~ normal(0, .25);
    kernel_lengthscale ~ inv_gamma(2.5,0.80);// inv_gamma for zero-avoiding prop
    alpha ~ inv_gamma(2.5,.80);             // kernel std for linear kernel
